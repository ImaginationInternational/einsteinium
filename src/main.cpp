...
#if defined(NDEBUG)
# error "Imagination cannot be compiled without assertions."
#endif
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s XIM additional fees, %d delta bytes\n",
...
void ThreadScriptCheck() {
    RenameThread("imagination-scriptch");
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (block.nVersion >= 2)
    {
        if (consensusParams.BIP34Height != -1)
        {
            // Mainnet - Imagination: enforce together with BIP66 because this part was forgotten in the previos code and V1 blocks are being mined, change can be ignored in future
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        if (memcmp(msg.hdr.pchMessageStart, chainparams.MessageStart(), MESSAGE_START_SIZE) != 0) {
            ...
            LogPrintf("msg.hdr.pchMessageStart   : %s\n", msg.hdr.pchMessageStart);
            LogPrintf("chainparams.MessageStart(): %s\n", chainparams.MessageStart());
            LogPrintf("MESSAGE_START_SIZE        : %s\n", MESSAGE_START_SIZE);
            fOk = false;
            break;
        }
